...
лица царя и опирается на реальные исторические документы. Текстовые источники
лежат в `sources/`.

## Что умеет прототип

- веб-чат с компактным интерфейсом в стиле петровской эпохи;
- Telegram-бот;
...
- показ найденных фрагментов-источников и имени файла в веб-интерфейсе.
...
Источники RAG по умолчанию читаются из папки:

```env
DOCUMENTS_PATH=sources
```
